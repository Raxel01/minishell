...
/*   Updated: 2023/10/17 16:46:06 by abait-ta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_var(char *readed_data, enum e_heredoc type, t_here_doc **head)
{
	...
	if (digit_case(readed_data[1]) || readed_data[1] == '?' || readed_data[1] == '$')
	{
		list_builder(head,creatnodes(ft_strndup(readed_data, 2), type));
		return (readed_data + 2);
	}
	len = 0;
	while (readed_data[len + 1] && (is_alphanum(readed_data[len + 1]) || readed_data[len + 1] == '_'))
		len++;
	if (len)
